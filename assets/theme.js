...
(function(){
  ...
  // Function to show the error message for 5 seconds
  function showErrorMessage(message) {
    ...
    // Append the error message element to the container
        ...
        theme.cart.changeItem(key, qty)
          .then(function(cart) {  
            ...
            if (parsedCart.status === 422) {
              showErrorMessage("Sorry! We don't have any more units for this item.");
              console.log(parsedCart.message); 
        ...
        <div class="grid__item grid-product ${gridWidth} aos-animate" data-aos="row-of-${rowOf}">
          <div class="grid-product__content">
            <a href="${product.url}" class="grid-product__link">
              <div class="grid-product__image-mask place-holder">
                ${image}
              </div>
  ...
  class ProductRecommendations extends HTMLElement {
    constructor() {
      ...
      document.addEventListener('recommendations:loaded', (e) => {
        ...
        /* COMPARE APP CALLBACK START */
        typeof compareGridRender != "undefined" && compareGridRender();
        /* COMPARE APP CALLBACK END */
...
// JavaScript to remove item and refresh minicart section only
document.addEventListener("click", async function (event) {
  const button = event.target.closest(".minicart-remove-item");
  if (!button) return;

  const iconImg = button.querySelector("img");
  if (iconImg) iconImg.classList.add("is-removing");
 
  const itemKey = button.getAttribute("data-key");
  const itemElement = button.closest("#CartDrawerForm");
  itemElement?.classList.add("is-updating");
 
  try {
    // Step 1: Update cart using Shopify API
    ...
    // Step 2: Re-fetch the cart drawer HTML
    const sectionUrl = `/?section_id=main-cart&_=${Date.now()}`;
    ...
    const updatedDrawer = doc.querySelector("#CartDrawerForm");
    const currentDrawer = document.querySelector("#CartDrawerForm");
    
    if (updatedDrawer && currentDrawer) {
      currentDrawer.replaceWith(updatedDrawer);
    } 
    setTimeout(() => {
      location.reload();
    }, 200);
